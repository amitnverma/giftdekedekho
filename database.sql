...
CREATE TABLE IF NOT EXISTS `ar_frames` (
  ...
  `channel`             ENUM('online','in_store','partner') NOT NULL DEFAULT 'online',
  `partner_id`          INT UNSIGNED DEFAULT NULL,            -- B2B partner content (channel 'partner')
  `partner_customer_id` INT UNSIGNED DEFAULT NULL,
  `title`               VARCHAR(160) DEFAULT NULL,
  `content_kind`        ENUM('single','album') DEFAULT NULL,
  `validity`            VARCHAR(10)  DEFAULT NULL,
  `active_until`        DATETIME     DEFAULT NULL,            -- NULL = no expiry
  ...
  KEY `idx_arf_partner`    (`partner_id`, `created_at`),
  CONSTRAINT `fk_arf_partner` FOREIGN KEY (`partner_id`)
    REFERENCES `ar_partners` (`id`) ON DELETE RESTRICT,
  CONSTRAINT `fk_arf_partner_customer` FOREIGN KEY (`partner_customer_id`)
...
CREATE TABLE IF NOT EXISTS `ar_frame_items` (
  ...
  `video_path`          VARCHAR(500) DEFAULT NULL,
  `playback_mode`       ENUM('fullscreen','overlay') NOT NULL DEFAULT 'fullscreen',
  `max_seconds`         SMALLINT UNSIGNED DEFAULT NULL,      -- partner content: playback stops here
  `trackability_score`  SMALLINT UNSIGNED DEFAULT NULL,
  `trackability_flag`   ENUM('poor','fair','good') DEFAULT NULL,
  `trackability_json`   JSON DEFAULT NULL,
...
-- ----------------------------
-- B2B AR partners (see migrations/2026_09_16_ar_partners.sql)
-- ----------------------------
